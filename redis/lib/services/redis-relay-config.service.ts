...
export class RedisRelayConfigService extends RedisConfigService {
    constructor() {
        ...
        this.port = process.env.REDIS_RELAY_PORT != undefined ? +process.env.REDIS_RELAY_PORT : 6379;
        this.database = process.env.REDIS_RELAY_DATABASE != undefined ? +process.env.REDIS_RELAY_DATABASE : 0;
        this.useTLS = process.env.REDIS_RELAY_USE_TLS === "true";
        this.lpushBlockSize =
            process.env.REDIS_RELAY_LPUSH_BLOCK_SIZE != undefined ? +process.env.REDIS_RELAY_LPUSH_BLOCK_SIZE : 250;
    }
}
