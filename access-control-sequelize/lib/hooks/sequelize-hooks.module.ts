...
export class SequelizeHooksAccessControlModule implements OnModuleInit {
    constructor(
        ...
        private readonly resourceEventAccessControlService: ResourceEventAccessControlService<any>
    ) {}

    public onModuleInit(): void {
        for (const model of this.sequelize.modelManager.all) {
            (model as unknown as typeof M).addHook(
                "afterCreate",
                "access-control-created-policy-hook",
                async (instance: M, options) => {
            ...
            (model as unknown as typeof M).addHook(
                "afterBulkCreate",
                "access-control-bulk-created-policy-hook",
                async (instances: M[], options) => {
                    if (options.transaction) {
                        options.transaction.afterCommit(async () => {
                            await Promise.all(
            ...
            (model as unknown as typeof M).addHook(
                "beforeBulkUpdate",
                "access-control-before-update-hook",
                (options) => {
            ...
            (model as unknown as typeof M).addHook(
                "afterUpdate",
                "access-control-updated-policy-hook",
                async (instance: M, options) => {
                    /**
                    ...
                    const run = async () => {
                        await this.resourceEventAccessControlService.onResourceUpdated(
                            `${model.tableName}-sequelize`,
                            previous,
                            instance
                        );
                    };
            ...
            (model as unknown as typeof M).addHook(
                "beforeBulkDestroy",
                "access-control-before-destroy-policy-hook",
                async (options) => {
                    options.individualHooks = true;
                }
            );
            ...
            (model as unknown as typeof M).addHook(
                "afterDestroy",
                "access-control-deleted-policy-hook",
                async (instance: M, options) => {
                    const run = async () => {
                        await this.resourceEventAccessControlService.onResourceDeleted(
                            `${model.tableName}-sequelize`,
                            instance,
